...
static void draw_play(void *instance) {
  ...
  if (btn->widget.disabled == true) {

    // Disabled
    blit_ex(btn->blitter, x, y, 4, 112, 113, 128, 129);
  } else {
    if (btn->widget.mouse.down == false) {
      if (btn->widget.mouse.over == false) {
...
Button btn_play_init(const struct Blitter *blitter, Uint32 event) {
  ...
  return (Button){.widget = widget, .blitter = blitter, .event = event};
}
...
static void draw_mute(void *instance) {
  ...
  if (sw->widget.mouse.down == false) {
    if (sw->widget.mouse.over == false) {
...
struct Switch sw_mute_init(const struct Blitter *blitter, Uint32 event,
                           const bool *value) {
  Widget widget = widget_init();
  ...
  return (struct Switch){
      .widget = widget, .blitter = blitter, .event = event, .value = value};
}
...
struct Switch sw_lock_init(const struct Blitter *blitter, Uint32 event,
                           const bool *value) {
  Widget widget = widget_init();
...
static void draw_track(void *instance) {
  ...
  SDL_Rect tmp_rect;
  SDL_GetRenderViewport(track->blitter->renderer, &tmp_rect);
  SDL_SetRenderViewport(track->blitter->renderer, &track->widget.rect);
  ...
  if (track->widget.mouse.over == true) {
    ...
    if (mouse_y >= 8 && mouse_y < 56 && mouse_x >= 0 && mouse_x < max_x) {
      int index = track->widget.mouse.down == true ? 22 : 40;
      blit(track->blitter, (int)(mouse_x / 8) * 8 - track->offset,
           (int)(mouse_y / 8) * 8, index);
    }
  }

  SDL_SetRenderViewport(track->blitter->renderer, &tmp_rect);
}
...
struct Track track_init(const struct Blitter *blitter, Uint32 event,
  ...
  return (struct Track){.widget = widget,
                        .blitter = blitter,
                        .event = event,
                        .pstate = pstate,
                        .offset = 0};
...
static void draw_pattern(void *instance) {
  ...
  SDL_Rect tmp_rect;
  SDL_GetRenderViewport(pat->blitter->renderer, &tmp_rect);
  SDL_SetRenderViewport(pat->blitter->renderer, &pat->widget.rect);
  ...
  for (int i = 0; i < 16; i++) {
    ...
    int pattern_index =
        (pat->pstate->counter % (pat->pstate->patterns * 16)) / 16;
  ...
  if (pat->widget.mouse.over == true) {
    ...
    if (mouse_x >= 0 && mouse_x < max_x) {
      ...
      if (pat->widget.mouse.down == true) {

        // Mouse down
        blit_ex(pat->blitter, x, 0, 4, 238, 239, 254,
                255); // must be empty space
  ...
  SDL_SetRenderViewport(pat->blitter->renderer, &tmp_rect);
}
...
struct Pattern pattern_init(const struct Blitter *blitter, Uint32 event,
                            const struct ProgramState *pstate) {
  Widget widget = widget_init();
  ...
  return (struct Pattern){.widget = widget,
                          .blitter = blitter,
                          .event = event,
                          .pstate = pstate,
                          .offset = 0};
